...
class ReferenceHelper
{
	...
	public function insertNewBefore($pBefore = 'A1', $pNumCols = 0, $pNumRows = 0, Worksheet $pSheet = null) {
		...
		while (($cellID = ($pNumCols < 0 || $pNumRows < 0) ? array_shift($aCellCollection) : array_pop($aCellCollection))) {
			...
			if (
					(Cell::columnIndexFromString( $cell->getColumn() ) >= Cell::columnIndexFromString($beforeColumn)) &&
	...
	public function updateNamedFormulas(PHPExcel $pPhpExcel, $oldName = '', $newName = '') {
		...
		foreach ($pPhpExcel->getWorksheetIterator() as $sheet) {
			foreach ($sheet->getCellCollection(false) as $cellID) {
				...
				if (!is_null($cell) && $cell->getDataType() == Cell_DataType::TYPE_FORMULA) {
	...
	private function _updateSingleCellReference($pCellReference = 'A1', $pBefore = 'A1', $pNumCols = 0, $pNumRows = 0) {
		if (strpos($pCellReference, ':') === false && strpos($pCellReference, ',') === false) {
			...
			$updateColumn = (Cell::columnIndexFromString($newColumn) >= Cell::columnIndexFromString($beforeColumn))
