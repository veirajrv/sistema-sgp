 ...
 * @copyright  Copyright (c) 2006 - 2010 PHPExcel (http://www.codeplex.com/PHPExcel)
 */
class Writer_Serialized implements Writer_IWriter
{
	/**
	 * Private PHPExcel
	 *
    ...
    /**
     * Create a new Writer_Serialized
     *
		...
		if (!is_null($this->_spreadSheet)) {
			...
			for ($i = 0; $i < $sheetCount; ++$i) {
				for ($j = 0; $j < $this->_spreadSheet->getSheet($i)->getDrawingCollection()->count(); ++$j) {
					if ($this->_spreadSheet->getSheet($i)->getDrawingCollection()->offsetGet($j) instanceof Worksheet_BaseDrawing) {
	...
	private function _writeSerialized(PHPExcel $pPHPExcel = null, $pFilename = '')
	{
		...
		for ($i = 0; $i < $sheetCount; ++$i) {
			for ($j = 0; $j < $pPHPExcel->getSheet($i)->getDrawingCollection()->count(); ++$j) {
				if ($pPHPExcel->getSheet($i)->getDrawingCollection()->offsetGet($j) instanceof Worksheet_BaseDrawing) {
