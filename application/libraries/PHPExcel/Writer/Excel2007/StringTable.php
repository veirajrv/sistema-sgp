		...
		if (!is_null($pSheet)) {
	        ...
	        foreach ($pSheet->getCellCollection() as $cellID) {
	        	...
	        	if (!is_object($cell->getValue()) &&
	        		...
	        		$cell->getValue() !== '' &&
	        		($cell->getDataType() == Cell_DataType::TYPE_STRING || $cell->getDataType() == Cell_DataType::TYPE_NULL)
	        	) {
	        			$aStringTable[] = $cell->getValue();
						$aFlippedStringTable[$cell->getValue()] = 1;

	        	} else if ($cell->getValue() instanceof RichText &&
